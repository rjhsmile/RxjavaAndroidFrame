...
//                    ex.message = "userName or passWord is error!";
                    ex.meg = App.getContext().getString(R.string.service_connect_failed);
                    break;
...
//                    ex.message = "服务器异常，请稍后再试";
                ...
                case ErrorType.REQUEST_TIMEOUT:
                    ex.meg = App.getContext().getString(R.string.connect_timeout);
//                    ex.message = "请求超时";
                    break;
                case ErrorType.GATEWAY_TIMEOUT:
//                    ex.message = "作为网关或者代理工作的服务器尝试执行请求时，未能及时从上游服务器（URI标识出的服务器，例如HTTP、FTP、LDAP" +
//                            "）或者辅助服务器（例如DNS）收到响应";
                    ex.meg = App.getContext().getString(R.string.server_exception);
                    break;
...
//                    ex.message = "由于临时的服务器维护或者过载，服务器当前无法处理请求";
                    ...
                    break;
                default:
                    ex.meg = App.getContext().getString(R.string.server_exception); // 其它均视为网络错误
                    break;
            }
            return ex;
